...
```javascript
// ✅ 原有用法继续工作
s.if(d => d.age >= 18).and(d => d.role === 'admin').message('不符合条件')

// ✅ .and() 后不调用 .message() 也可以
s.if(d => !d).message('整体错误').and(d => d < 100).assert(50)
// → 使用整体消息 '整体错误'
```
...
`s.if()` 提供两种使用方式，根据参数类型自动选择：

| 方式 | 参数类型 | 执行时机 | 用途 | 示例 |
|------|---------|---------|------|------|
| **方式一** | 字符串 | Schema 定义时 | 静态布尔条件 | `s.if('isVip', thenSchema, elseSchema)` |
| **方式二** | 函数 | 验证时 | 动态条件判断 | `s.if((data) => data.age >= 18).then(...)` |
...
// 4. 复用验证器
const ageValidator = s.if(d => d.age < 18).message('未成年用户不能注册');
...
s.if((data) => data.status === 'active' && data.verified)
```
...
// 复用验证器
const ageValidator = s.if(d => d.age < 18).message('未成年');
...
function validateAndCreate(data) {
  s.if(d => d.age < 18).message('未成年').assert(data);
  s.if(d => !d.email).message('邮箱必填').assert(data);
  s.if(d => !d.username).message('用户名必填').assert(data);
...
const adults = users.filter(u => 
  !s.if(d => d.age < 18).message('未成年').check(u)
);
```
...
// 创建可复用的验证器
const validators = {
  age: s.if(d => d.age < 18).message('未成年用户不能注册'),
...
**示例入口**: [conditional-api.ts](https://github.com/devcodex-labs/schema-dsl/blob/main/examples/docs/conditional-api.ts)  
**说明**: 同时覆盖失败谓词模式下的 `.check()` / `.assert()`，以及字段名版本 `s.if(field, then, else)` 和 `s.match()` 映射。
